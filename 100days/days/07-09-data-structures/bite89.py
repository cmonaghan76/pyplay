...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    """Get the first 10 state abbreviations ('AL', 'AK', 'AZ', ...) from
       ...
       alphabetical order"""
    # get the last 10 items from the states list
    # then sort the states list into alpa order,
    # then retrieve the first 10 states' abbreviations
    # from the abbrev dict
    combined = list()
    states_alpha = sorted(states)
    abbrev_alpha = sorted(us_state_abbrev.values())
    for x in range(10):
        combined.append(abbrev_alpha[x])
    for x in range(10):
        n = x-10
        combined.append(states_alpha[n])
    return combined
